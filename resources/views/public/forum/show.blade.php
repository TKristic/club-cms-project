...
<div class="max-w-2xl mx-auto">
    <a href="{{ route('forum.index') }}" class="text-sm text-gray-400 hover:underline">← {{ __('messages.back_to_topics') }}</a>
    ...
    @auth
        <form method="POST" action="{{ route('forum.storePost', $topic) }}"
            ...
            @csrf
            <textarea name="body" rows="3" placeholder="{{ __('messages.reply') }}..."
                      class="w-full border rounded-lg px-3 py-2" required></textarea>
            <button class="px-4 py-2 rounded-lg font-semibold text-white"
                    style="background: var(--klub-primarna)">{{ __('messages.reply') }}i</button>
        </form>
    @else
        <p class="text-sm text-gray-500 mt-6">
            <a href="{{ route('login') }}" class="underline">{{ __('messages.login') }}</a> {{ __('messages.login_to_reply') }}
        </p>
    @endauth
</div>
@endsection